...
I will not oversell it. The novel and risky parts are done and proven: the semantics, the guardrails, the wire path, the deploy model. That is real, and it is the hard 70 percent.
...
But the shape is right. One definition in git, reachable by every consumer, with the guardrails enforced in one place and bypassable by none. An analyst drags a semi-additive balance onto a chart and gets the end-of-month number. An LLM asks the same question and gets the same number. And when someone edits the YAML, a gate catches the mistake before it ships, and the fix rolls out without dropping a connection.

That was the thing I wanted to exist. Now it mostly does. The rest is on [GitHub](https://github.com/dbose/flowproxy), warts and roadmap included.
