...
for f in files:
    ...
    if file_type in ("qmd", "md"):
        # render link to html
        href = f"{name}.html"
        note_cards.append(card_html(display_name, href, "markdown", target="_self"))

    elif file_type == "pdf":
        # Ownership rule: if <pdf-name>.meta.yml exists -> "mine", else reference
        meta = HERE / f"{name}.meta.yml"
        is_mine = meta.exists()

        # Optional companion notes page (if exists, show subtitle / link still goes to pdf)
        companion = None
        if (HERE / f"{name}.qmd").exists() or (HERE / f"{name}.md").exists():
            companion = f"Notes: {name}.html"
